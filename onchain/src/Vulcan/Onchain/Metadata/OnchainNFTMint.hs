...
module Vulcan.Onchain.Metadata.OnchainNFTMint where
...
pvalidateNFTMint :: Term s (PTxOutRef :--> PNFTMintAction :--> PScriptContext :--> PUnit)
    ...
    pure $
      pif
        ( pmatch
            ...
            ( \case
                PMintNFT _ ->
                  let mintNoRoyaltyCheck = unTermCont $ do
                   ...
                   in pcond
                        [ (numMinted #== 2, mintNoRoyaltyCheck)
                        , (numMinted #== 3, mintCheck)
                        ]
                        perror
                PBurnNFT _ ->
